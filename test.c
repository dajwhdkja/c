...
// Default values
#define DEFAULT_PACKET_SIZE 4
#define DEFAULT_THREAD_COUNT 800
...
int main(int argc, char *argv[]) {
    ...
    if (local->tm_year + 1900 > EXPIRATION_YEAR ||
        ...
        printf("Program expired. Exiting.\n");
        return EXIT_FAILURE;
    }

    printf("Program active. Proceeding...\n");

    // Check for correct arguments
    if (argc < 4) {
        printf("Usage: %s [IP] [PORT] [DURATION] [PACKET_SIZE (optional)] [THREAD_COUNT (optional)]\n", argv[0]);
        return -1;
    }
    ...
    // Validate the input
    if (target_port <= 0 || duration <= 0 || packet_size <= 0 || thread_count <= 0) {
        printf("Invalid input: Ensure PORT, DURATION, PACKET_SIZE, and THREAD_COUNT are positive integers.\n");
        return -1;
    }
